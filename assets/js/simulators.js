...
function setupgroup(){
  ...
  const defaultLabels=Array.from({length:21},(_,i)=>`Participant ${i+1}`);
  let participantLabels=loadParticipantLabels();
  function loadParticipantLabels(){
    try{const saved=JSON.parse(localStorage.getItem("slip39ParticipantLabels"));if(Array.isArray(saved)&&saved.length===21)return saved.map((x,i)=>String(x||defaultLabels[i]));}catch(e){}
    return [...defaultLabels];
  }
  const gneed=document.getElementById("groupNeed"),mneed=document.getElementById("memberNeed"),status=document.getElementById("participantStatus"),detail=document.getElementById("participantDetail");
  function render(){
    ...
    for(let g=0;g<7;g++){
      ...
      for(let i=start;i<start+3;i++){const d=document.createElement("div");d.className="member "+(active[i]?"on":"off");d.innerHTML=`<span>${escapeHtml(participantLabels[i])}</span><span class="pill">${active[i]?"available":"unavailable"}</span>`;d.onclick=()=>{active[i]=!active[i];render()};box.appendChild(d)}
...
function setupLocalLabels(){
  const modal=document.getElementById("labelsModal");
  if(!modal)return;

  const openBtn=document.getElementById("openLocalLabels");
  const closeBtn=document.getElementById("closeLocalLabels");
  const grid=document.getElementById("participantLabelFields");
  const message=document.getElementById("labelsMessage");

  const defaults=Array.from({length:21},(_,i)=>`Participant ${i+1}`);

  function getNames(){
    try{
      const saved=JSON.parse(localStorage.getItem("slip39ParticipantLabels"));
  ...
  function renderFields(){
    ...
    names.forEach((name,index)=>{
      ...
      wrap.className="labels-field";
      wrap.innerHTML=`<label>Participant ${index+1}</label><input maxlength="60" data-name-index="${index}" value="${escapeHtml(name)}">`;
  ...
  document.getElementById("saveParticipantLabels")?.addEventListener("click",()=>{
    const names=[...grid.querySelectorAll("input[data-name-index]")].map((input,i)=>input.value.trim()||defaults[i]);
    localStorage.setItem("slip39ParticipantLabels",JSON.stringify(names));
    message.textContent="Saved locally in this browser.";
    location.reload();
  });

  document.getElementById("resetParticipantLabels")?.addEventListener("click",()=>{
    localStorage.removeItem("slip39ParticipantLabels");
    renderFields();
    message.textContent="Names reset to Participant 1–21.";
  });

  document.getElementById("exportParticipantLabels")?.addEventListener("click",()=>{
    const names=[...grid.querySelectorAll("input[data-name-index]")].map((input,i)=>input.value.trim()||defaults[i]);
    const blob=new Blob([JSON.stringify({version:1,names},null,2)],{type:"application/json"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");
    a.href=url;a.download="slip39-participant-labels.json";a.click();
    URL.revokeObjectURL(url);
    message.textContent="JSON exported. It contains only the labels shown here.";
  });

  document.getElementById("importParticipantLabels")?.addEventListener("change",(event)=>{
    ...
    reader.onload=()=>{
      try{
        ...
        localStorage.setItem("slip39ParticipantLabels",JSON.stringify(names.map((x,i)=>String(x||defaults[i]))));
        renderFields();
...
document.addEventListener("DOMContentLoaded",()=>{setupSimple();setupgroup();setupLocalLabels()});
